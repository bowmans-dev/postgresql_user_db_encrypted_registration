...
app.post("/login", async (req, res) => {
  const sql = "select * from auth_method where username = $1"
...
app.post("/register", async (req, res) => {
  //check if user exist 
  const sql = "select username from auth_method where username = $1"
  const result = await pool.query(sql,
    [req.body.user]);
  ...
  if (result.rowCount === 0)
  {
    ...
    await pool.query("insert into auth_method (username, password, salt) values ($1,$2, $3)",
...
app.listen(port, () => 
console.log("Auth Method - Listening on " + port))
